...
// Update specific elements - Simplified version
function updateSpecificElements() {
    console.log('Updating specific elements for language:', currentLanguage);
    
    // Force refresh all text content
    if (currentLanguage === 'fa') {
        ...
        const navTexts = ['خانه', 'درباره من', 'مهارت‌ها', 'تحصیلات', 'نمونه کارها', 'تماس'];
        ...
        if (titleLines[0]) titleLines[0].textContent = 'خوش آمدید به';
        if (titleLines[1]) titleLines[1].textContent = 'وب‌سایت من!';
        
        const subtitleLines = document.querySelectorAll('.subtitle-line');
        if (subtitleLines[0]) subtitleLines[0].textContent = 'برنامه‌نویس';
        if (subtitleLines[1]) subtitleLines[1].textContent = 'فرانت‌اند';
        
        // Hero buttons
        const heroButtons = document.querySelectorAll('.hero-buttons .btn');
        if (heroButtons[0]) heroButtons[0].textContent = 'نمونه کارهای من';
        if (heroButtons[1]) heroButtons[1].textContent = 'تماس با من';
        
        // Section titles
        const sectionTitles = document.querySelectorAll('.section-title');
        const titles = ['درباره من', 'مهارت‌های من', 'تحصیلات و گواهینامه‌ها', 'نمونه کارهای من', 'تماس با من'];
        ...
        if (aboutDesc) {
            aboutDesc.textContent = 'با بیش از 3 سال تجربه در توسعه فرانت‌اند و بک‌اند، مهارت‌های پیشرفته‌ای در HTML، CSS، JavaScript، React.js و Node.js دارم. در فریمورک‌های مدرن تخصص دارم و تجربه گسترده‌ای با Next.js، Vue.js و TypeScript دارم. همچنین در فریمورک‌های استایل‌دهی مانند Tailwind CSS و Sass مهارت دارم. توانایی کار تیمی قوی و تجربه گسترده در استانداردهای بهینه‌سازی وب و بهترین شیوه‌ها دارم.';
        }
        
        // About traits
        const traitElements = document.querySelectorAll('.trait');
        const traits = ['علاقه‌مند به یادگیری', 'پرشور', 'حل‌کننده مسئله', 'خلاق', 'کار تیمی', 'انطباق‌پذیر'];
        traitElements.forEach((trait, index) => {
            if (traits[index]) {
                trait.textContent = traits[index];
            }
        });
        ...
        if (profileName) {
            profileName.textContent = 'Parsa Nikoonahad';
        }
        if (profileTitle) {
            profileTitle.textContent = 'برنامه‌نویس فرانت‌اند';
        }
        
        // Skills categories
        const skillCategories = document.querySelectorAll('.skill-category h3');
        if (skillCategories[0]) skillCategories[0].textContent = 'فرانت‌اند';
        if (skillCategories[1]) skillCategories[1].textContent = 'ابزارها و تکنولوژی‌ها';
        
        // Education items
        const educationItems = document.querySelectorAll('.timeline-item');
        if (educationItems[0]) {
            const title = educationItems[0].querySelector('.timeline-title');
            const company = educationItems[0].querySelector('.timeline-company');
            const year = educationItems[0].querySelector('.timeline-year');
            const desc = educationItems[0].querySelector('.timeline-description');
            if (title) title.textContent = 'کارشناسی مهندسی کامپیوتر';
            if (company) company.textContent = 'دانشگاه تهران';
            if (year) year.textContent = '1398 - 1402';
            if (desc) desc.textContent = 'تحصیل در رشته مهندسی کامپیوتر با گرایش نرم‌افزار. تمرکز بر توسعه وب و الگوریتم‌های کامپیوتری.';
        }
        if (educationItems[1]) {
            const title = educationItems[1].querySelector('.timeline-title');
            const company = educationItems[1].querySelector('.timeline-company');
            const year = educationItems[1].querySelector('.timeline-year');
            const desc = educationItems[1].querySelector('.timeline-description');
            if (title) title.textContent = 'گواهینامه React.js';
            if (company) company.textContent = 'Meta (فیس‌بوک)';
            if (year) year.textContent = '1401';
            if (desc) desc.textContent = 'تکمیل دوره تخصصی React.js از Meta شامل Hooks، Context API و بهترین شیوه‌های توسعه.';
        }
        if (educationItems[2]) {
            const title = educationItems[2].querySelector('.timeline-title');
            const company = educationItems[2].querySelector('.timeline-company');
            const year = educationItems[2].querySelector('.timeline-year');
            const desc = educationItems[2].querySelector('.timeline-description');
            if (title) title.textContent = 'گواهینامه JavaScript';
            if (company) company.textContent = 'freeCodeCamp';
            if (year) year.textContent = '1400';
            if (desc) desc.textContent = 'تکمیل دوره کامل JavaScript شامل ES6+، DOM Manipulation و API Integration.';
        }
        
        // Portfolio items
        const portfolioItems = document.querySelectorAll('.portfolio-item');
        if (portfolioItems[0]) {
            const title = portfolioItems[0].querySelector('.portfolio-title');
            const desc = portfolioItems[0].querySelector('.portfolio-description');
            const btn = portfolioItems[0].querySelector('.portfolio-btn');
            if (title) title.textContent = 'پلتفرم تجارت الکترونیک';
            if (desc) desc.textContent = 'پلتفرم خرید آنلاین ساخته شده با Next.js برای فرانت‌اند، Redux برای مدیریت state و Firebase برای بک‌اند. طراحی واکنش‌گرا با Tailwind CSS.';
            if (btn) btn.textContent = 'مشاهده پروژه';
        }
        if (portfolioItems[1]) {
            const title = portfolioItems[1].querySelector('.portfolio-title');
            const desc = portfolioItems[1].querySelector('.portfolio-description');
            const btn = portfolioItems[1].querySelector('.portfolio-btn');
            if (title) title.textContent = 'اپلیکیشن ToDo';
            if (desc) desc.textContent = 'اپلیکیشن مدیریت کارها با React.js و TypeScript. شامل API های مختلف و استفاده از React Router DOM برای مسیریابی. طراحی واکنش‌گرا با Tailwind CSS.';
            if (btn) btn.textContent = 'مشاهده پروژه';
        }
        
        // Portfolio tech stacks
        const techStacks = document.querySelectorAll('.portfolio-tech');
        techStacks.forEach(techStack => {
            const techItems = techStack.querySelectorAll('.tech-item');
            techItems.forEach(item => {
                const text = item.textContent.trim();
                if (text === 'Next.js') item.textContent = 'Next.js';
                else if (text === 'Redux') item.textContent = 'Redux';
                else if (text === 'Firebase') item.textContent = 'Firebase';
                else if (text === 'Tailwind CSS') item.textContent = 'Tailwind CSS';
                else if (text === 'React.js') item.textContent = 'React.js';
                else if (text === 'TypeScript') item.textContent = 'TypeScript';
                else if (text === 'React Router') item.textContent = 'React Router';
            });
        });
        
        // Contact items
        const contactItems = document.querySelectorAll('.contact-item');
        const contactTitles = ['ایمیل', 'تلفن', 'تلگرام', 'اینستاگرام'];
        contactItems.forEach((item, index) => {
            const title = item.querySelector('.contact-title');
            if (title && contactTitles[index]) {
                title.textContent = contactTitles[index];
            }
        });
        ...
        if (nameInput) nameInput.placeholder = 'نام شما';
        if (emailInput) emailInput.placeholder = 'ایمیل شما';
        if (messageTextarea) messageTextarea.placeholder = 'پیام شما';
        if (submitButton) submitButton.textContent = 'ارسال پیام';
        
        // Footer
        const footerText = document.querySelector('.footer p');
        if (footerText) {
            footerText.innerHTML = '&copy; ۱۴۰۳ پارسا نیکونهاد. تمامی حقوق محفوظ است.';
        }
        ...
        // Navigation
        const navLinks = document.querySelectorAll('.nav-link');
        const navTexts = ['Home', 'About', 'Skills', 'Education', 'Portfolio', 'Contact'];
        navLinks.forEach((link, index) => {
            if (navTexts[index]) {
                link.textContent = navTexts[index];
            }
        ...
        // Hero section
        const titleLines = document.querySelectorAll('.title-line');
        if (titleLines[0]) titleLines[0].textContent = 'Welcome To';
        if (titleLines[1]) titleLines[1].textContent = 'My Website!';
        
        const subtitleLines = document.querySelectorAll('.subtitle-line');
        if (subtitleLines[0]) subtitleLines[0].textContent = 'Front-End';
        if (subtitleLines[1]) subtitleLines[1].textContent = 'Web Developer';
        
        // Hero buttons
        const heroButtons = document.querySelectorAll('.hero-buttons .btn');
        if (heroButtons[0]) heroButtons[0].textContent = 'View My Work';
        if (heroButtons[1]) heroButtons[1].textContent = 'Get In Touch';
        
        // Section titles
        const sectionTitles = document.querySelectorAll('.section-title');
        const titles = ['About Me', 'My Skills', 'Education & Certifications', 'My Portfolio', 'Contact Me Anytime'];
        sectionTitles.forEach((title, index) => {
            if (titles[index]) {
                title.textContent = titles[index];
            }
        ...
        if (profileTitle) {
            profileTitle.textContent = 'Full Stack Developer';
        }
        
        // Skills categories
        const skillCategories = document.querySelectorAll('.skill-category h3');
        if (skillCategories[0]) skillCategories[0].textContent = 'Frontend';
        if (skillCategories[1]) skillCategories[1].textContent = 'Tools & Technologies';
        ...
        if (educationItems[0]) {
            ...
            const year = educationItems[0].querySelector('.timeline-year');
            const desc = educationItems[0].querySelector('.timeline-description');
            if (title) title.textContent = 'Bachelor\'s Degree in Computer Engineering';
            if (company) company.textContent = 'University of Tehran';
            if (year) year.textContent = '2019 - 2023';
            if (desc) desc.textContent = 'Studied Computer Engineering with a focus on Software Engineering. Concentrated on web development and computer algorithms.';
        }
        if (educationItems[1]) {
            const title = educationItems[1].querySelector('.timeline-title');
            const company = educationItems[1].querySelector('.timeline-company');
            const year = educationItems[1].querySelector('.timeline-year');
            const desc = educationItems[1].querySelector('.timeline-description');
            if (title) title.textContent = 'React.js Certification';
            if (company) company.textContent = 'Meta (Facebook)';
            if (year) year.textContent = '2022';
            if (desc) desc.textContent = 'Completed specialized React.js course from Meta including Hooks, Context API, and best development practices.';
        }
        if (educationItems[2]) {
            const title = educationItems[2].querySelector('.timeline-title');
            const company = educationItems[2].querySelector('.timeline-company');
            const year = educationItems[2].querySelector('.timeline-year');
            const desc = educationItems[2].querySelector('.timeline-description');
            if (title) title.textContent = 'JavaScript Certification';
            if (company) company.textContent = 'freeCodeCamp';
            if (year) year.textContent = '2021';
            if (desc) desc.textContent = 'Completed comprehensive JavaScript course including ES6+, DOM Manipulation, and API Integration.';
        }
        ...
        if (portfolioItems[0]) {
            ...
            const desc = portfolioItems[0].querySelector('.portfolio-description');
            const btn = portfolioItems[0].querySelector('.portfolio-btn');
            if (title) title.textContent = 'E-Commerce Platform';
            if (desc) desc.textContent = 'Online shopping platform built with Next.js for frontend, Redux for state management, and Firebase for backend. Responsive design with Tailwind CSS.';
            if (btn) btn.textContent = 'View Project';
        }
        if (portfolioItems[1]) {
            const title = portfolioItems[1].querySelector('.portfolio-title');
            const desc = portfolioItems[1].querySelector('.portfolio-description');
            const btn = portfolioItems[1].querySelector('.portfolio-btn');
            if (title) title.textContent = 'Todo Application';
            if (desc) desc.textContent = 'Task management application built with React.js and TypeScript. Features various APIs and React Router DOM for navigation. Responsive design with Tailwind CSS.';
            if (btn) btn.textContent = 'View Project';
        }
        
        // Contact items
        const contactItems = document.querySelectorAll('.contact-item');
        const contactTitles = ['Email', 'Phone', 'Telegram', 'Instagram'];
        contactItems.forEach((item, index) => {
            const title = item.querySelector('.contact-title');
            if (title && contactTitles[index]) {
                title.textContent = contactTitles[index];
        ...
        const footerText = document.querySelector('.footer p');
        if (footerText) {
            footerText.innerHTML = '&copy; 2024 Parsa Nikoonahad. All rights reserved.';
        }
    }
    
    console.log('Specific elements updated successfully');
}

// Force refresh all elements
function forceRefreshAll() {
    console.log('Force refreshing all elements...');
    
    // Force refresh by removing and re-adding classes
...
// Translate all text elements that might be missed
function translateAllTextElements() {
    console.log('Translating all text elements...');
    ...
    while (node = walker.nextNode()) {
        ...
        // Translate specific English texts to Persian
        if (currentLanguage === 'fa') {
            if (text === 'Eager Learner') node.textContent = 'علاقه‌مند به یادگیری';
            else if (text === 'Passionate') node.textContent = 'پرشور';
            else if (text === 'Problem Solver') node.textContent = 'حل‌کننده مسئله';
            else if (text === 'Creative') node.textContent = 'خلاق';
            else if (text === 'Team Player') node.textContent = 'کار تیمی';
            else if (text === 'Adaptable') node.textContent = 'انطباق‌پذیر';
            else if (text === 'Full Stack Developer') node.textContent = 'برنامه‌نویس فرانت‌اند';
            else if (text === 'Bachelor\'s Degree in Computer Engineering') node.textContent = 'کارشناسی مهندسی کامپیوتر';
            else if (text === 'University of Tehran') node.textContent = 'دانشگاه تهران';
            else if (text === 'React.js Certification') node.textContent = 'گواهینامه React.js';
            else if (text === 'Meta (Facebook)') node.textContent = 'Meta (فیس‌بوک)';
            else if (text === 'JavaScript Certification') node.textContent = 'گواهینامه JavaScript';
            else if (text === 'E-Commerce Platform') node.textContent = 'پلتفرم تجارت الکترونیک';
            else if (text === 'Todo Application') node.textContent = 'اپلیکیشن ToDo';
            else if (text === 'View Project') node.textContent = 'مشاهده پروژه';
            else if (text === 'Email') node.textContent = 'ایمیل';
            else if (text === 'Phone') node.textContent = 'تلفن';
            else if (text === 'Telegram') node.textContent = 'تلگرام';
            else if (text === 'Instagram') node.textContent = 'اینستاگرام';
            else if (text === 'Online shopping platform built with Next.js for frontend, Redux for state management, and Firebase for backend. Responsive design with Tailwind CSS.') node.textContent = 'پلتفرم خرید آنلاین ساخته شده با Next.js برای فرانت‌اند، Redux برای مدیریت state و Firebase برای بک‌اند. طراحی واکنش‌گرا با Tailwind CSS.';
            else if (text === 'Task management application built with React.js and TypeScript. Features various APIs and React Router DOM for navigation. Responsive design with Tailwind CSS.') node.textContent = 'اپلیکیشن مدیریت کارها با React.js و TypeScript. شامل API های مختلف و استفاده از React Router DOM برای مسیریابی. طراحی واکنش‌گرا با Tailwind CSS.';
        } else {
            // Translate Persian texts to English
            if (text === 'علاقه‌مند به یادگیری') node.textContent = 'Eager Learner';
            else if (text === 'پرشور') node.textContent = 'Passionate';
            else if (text === 'حل‌کننده مسئله') node.textContent = 'Problem Solver';
            else if (text === 'خلاق') node.textContent = 'Creative';
            else if (text === 'کار تیمی') node.textContent = 'Team Player';
            else if (text === 'انطباق‌پذیر') node.textContent = 'Adaptable';
            else if (text === 'برنامه‌نویس فرانت‌اند') node.textContent = 'Full Stack Developer';
            else if (text === 'کارشناسی مهندسی کامپیوتر') node.textContent = 'Bachelor\'s Degree in Computer Engineering';
            else if (text === 'دانشگاه تهران') node.textContent = 'University of Tehran';
            else if (text === 'گواهینامه React.js') node.textContent = 'React.js Certification';
            else if (text === 'Meta (فیس‌بوک)') node.textContent = 'Meta (Facebook)';
            else if (text === 'گواهینامه JavaScript') node.textContent = 'JavaScript Certification';
            else if (text === 'پلتفرم تجارت الکترونیک') node.textContent = 'E-Commerce Platform';
            else if (text === 'اپلیکیشن ToDo') node.textContent = 'Todo Application';
            else if (text === 'مشاهده پروژه') node.textContent = 'View Project';
            else if (text === 'ایمیل') node.textContent = 'Email';
            else if (text === 'تلفن') node.textContent = 'Phone';
            else if (text === 'تلگرام') node.textContent = 'Telegram';
            else if (text === 'اینستاگرام') node.textContent = 'Instagram';
            else if (text === 'پلتفرم خرید آنلاین ساخته شده با Next.js برای فرانت‌اند، Redux برای مدیریت state و Firebase برای بک‌اند. طراحی واکنش‌گرا با Tailwind CSS.') node.textContent = 'Online shopping platform built with Next.js for frontend, Redux for state management, and Firebase for backend. Responsive design with Tailwind CSS.';
            else if (text === 'اپلیکیشن مدیریت کارها با React.js و TypeScript. شامل API های مختلف و استفاده از React Router DOM برای مسیریابی. طراحی واکنش‌گرا با Tailwind CSS.') node.textContent = 'Task management application built with React.js and TypeScript. Features various APIs and React Router DOM for navigation. Responsive design with Tailwind CSS.';
        }
    }
}
...
langBtn.addEventListener('click', function() {
    ...
    // Force refresh and apply translations
    forceRefreshAll();
